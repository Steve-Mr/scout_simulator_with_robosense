...
async function readPixels(file, fileSize, offset, pixelSize, signature) {
  ...
  if (signature == SIGNATURES.P5) {
    const BUFFER_SIZE = 1024 * 4 * pixelSize
    let buffer = Buffer.alloc(BUFFER_SIZE)

    for (let idx = offset; idx < fileSize;) {
      let readSize = BUFFER_SIZE
      if (fileSize - idx < BUFFER_SIZE) {
        readSize = fileSize - idx
      }

      await util.promisify(fs.read)(file, buffer, 0, readSize, idx)
      for (let i = 0; i < readSize; i = i + pixelSize) {
        pixels.push(buffer.readUIntBE(i, pixelSize))
      }
      idx = idx + readSize / pixelSize
    }
